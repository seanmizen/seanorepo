...
- **Rationale:** `visibility` is who may EVER see it. `published_at` is whether
  ...
  ambiguous. The composed rule is: the owner can always see it. For anybody
  else the brief must be published, and then `public` is visible to anyone,
  `link` to anyone who reaches the URL, and `private` to users on the invitee
  list. A viewer who may not see a brief gets the same answer as one asking for
  a brief that does not exist — a distinguishable
...
- **Verification:**
  ...
  - Test — `apps/inside/inside-be/src/tests/briefs.test.ts` › "an invitee sees a private brief, and a designer may be invited"
