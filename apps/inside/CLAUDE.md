...
- Import `createTestEnv()` from `./setup` **before** importing anything from
  `src/`. Server and storage modules read env at module scope, so ordering
  matters. Use a top-level `await import(...)` after it.
- Every suite gets its own temp dir for the database and uploads. Tests must
  never create `database.db` or `uploads/` in the repo — there is a check for
  this, and a leak means your setup is wrong.
