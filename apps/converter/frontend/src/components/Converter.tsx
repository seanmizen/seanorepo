...
interface ConverterProps {
  // When set, only files with this extension are accepted and the format
  // picker is hidden — landing pages use this to pre-select a conversion pair.
  presetInputExt?: string;
...
export function Converter({ presetInputExt, presetOutputFormat }: ConverterProps) {
  ...
  const handleFile = (file: File) => {
    ...
    if (presetInputExt && ext !== presetInputExt) {
      setStage({
        name: 'error',
        message: `This page only accepts .${presetInputExt} files. You dropped .${ext || '???'}.`,
      });
      return;
    }
    ...
    if (presetOutputFormat) {
      if (!formats.includes(presetOutputFormat)) {
        setStage({
          name: 'error',
          message: `Cannot convert .${ext} to .${presetOutputFormat}.`,
        });
        return;
      }
      setSelectedFmt(presetOutputFormat);
    } else {
