---
paths:
  - "utils/swindowzig/examples/voxel/**"
---

# swindowzig voxel demo

Read with `utils/swindowzig/CLAUDE.md`. The mandatory tests are there.
...
| `--fxaa-quality=<tier>` | FXAA 3.11 quality tier. Accepted: `low` (4-step edge search, 0.5 subpix cap, 0.166 edge threshold), `medium` (12-step, 0.75, 0.125 — **default**. Matches the pre-quality-tier baseline so old FXAA diffs stay valid), `high` (32-step, 0.85, 0.063). Tiers map to roughly FXAA 3.11 presets 12/25/39. A change in the Settings menu at runtime rebuilds the FXAA shader and pipeline. WGSL has no specialisation constants, so `configureFXAA` substitutes the tier into `fxaa.wgsl` by text replacement. |
...
| `--meshing=<mode>` | Mesher strategy. Accepted: `naive` (one quad per visible block face — preserves the `quad_block` parallel-array invariant used by `updateForBlockChange`) and `greedy` (the default). Greedy merges coplanar faces with the same material, AO and skylight into larger rectangles: about 35× fewer quads on flatland and 3× on hilly. Greedy quads span multiple blocks, so `updateForBlockChange` cannot patch them. A dig or place event sets `mesh_dirty = true` on the chunk, and the next tick rebuilds the whole mesh. Merge dimension capped at 6 blocks to stay within the painter's-algorithm sort tolerance (see `examples/voxel/docs/memory.md` §Rank 5). |
...
| `--frustum=<mode>` | Per-chunk cull strategy. Accepted: `none`, `sphere` and `cone`. `none` is the default, because culling is opt-in: it draws every loaded chunk. `sphere` is a cheap radial cutoff at render_distance plus slack. `cone` tests each sphere against a cone along the camera's forward vector, with the FOV set by `--frustum-fov-deg`. No strategy ever culls the camera's own chunk or its 8 horizontal neighbours. `examples/voxel/frustum.zig` has the math notes and edge-case tests. Cmd+F (Ctrl+F on Win/Linux) freezes the live frustum at its current transform so you can fly around and see what got culled. Settings menu has a live picker for the strategy. |
...
- `tests/cave_skylight.tas` — flatland 1×6×1 shaft, used to verify
  ...
  affected pixels are exactly the cave interior, and the sky region is unchanged).
...
**Summary: our code already does the right thing. The remaining lag is macOS, not us.**
...
## Game state and layers

- `game_state.zig` holds pure state logic with no GPU dependencies, and a fixed
  `[8]Layer` array.
- `overlay.zig` is a 2D alpha-blended pipeline. `ensurePipeline` creates it on
  first use. Each frame calls `begin`, then `rect`, then `draw`.
- Alpha blending on the native path in `gpu.zig`: allocate `[8]WGPUBlendState`
  on the stack, and set the pointer before you create the pipeline.
- Input gating: `gameplayReceivesInput()` blocks WASD and mouse look while the
  pause menu is open.
- Take the `was_captured` snapshot before the global key handling. This stops
  the camera jumping on the tick the game resumes.
