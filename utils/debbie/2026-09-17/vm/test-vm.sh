...
# Everything architecture- or run-specific lives here, so the tracked preseed
# stays free of conditionals - REQ-EMU-005.
write_overrides() {
    cat <<EOF
# GENERATED per run by test-vm.sh. Layered over preseed.cfg via preseed/include.
d-i passwd/username string $DEPLOY_USER
d-i netcfg/get_hostname string $SERVER_NAME
d-i netcfg/hostname string $SERVER_NAME
d-i debian-installer/add-kernel-opts string console=$CONSOLE,115200n8
...
d-i preseed/late_command string \\
    ...
    echo '$(cat "$KEY.pub")' > /target/home/$DEPLOY_USER/.ssh/authorized_keys; \\
